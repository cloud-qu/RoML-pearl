...
from MPModel.trainer_risklearner import RiskLearnerTrainer
...
class MP_BatchSampler(BatchSampler):
    ...
    def get_array_tasks(self, tasks):
        array_tasks = []
        for idx, task in enumerate(tasks):
            array_tasks.append(task[list(task.keys())[0]])
        return array_tasks
    
    def sample_tasks(self, i_iter, num_tasks, init_dist=None, test=False):
        if self.current_epoch < self.warmup:
            candidate_tasks = self._env.sample_tasks(num_tasks)
            array_tasks = self.get_array_tasks(candidate_tasks)
            self.current_epoch += 1
            return candidate_tasks, array_tasks
        candidate_tasks = self._env.sample_tasks(int(num_tasks*self.args.sample_ratio))
        ...
        if self.args.add_random:
            for i in range(len(candidate_tasks)):
                if not self.args.random_repeat:
                    ...
                    if len(tasks) >= num_tasks:
                        break
                    tasks.append(candidate_tasks[i])
        array_tasks = self.get_array_tasks(tasks)
        return tasks, array_tasks
...
class Diverse_MP_BatchSampler(MP_BatchSampler):
    def __init__(self, args,risk_learner_trainer, gamma_0, gamma_1, gamma_2, env_name, batch_size, device, seed, num_workers=mp.cpu_count() - 1):
        self.gamma_2 = gamma_2
        super(Diverse_MP_BatchSampler, self).__init__(args,risk_learner_trainer, gamma_0, gamma_1, env_name, batch_size, device, seed, num_workers)

    def get_acquisition_score(self, i_iter, tasks, diversified=False, real_batch_size=None):
        candidate_identifier = self.identifier_preprocess(tasks)
        if diversified:
            best_batch_id, diversified_score, combine_local_diverse_score, combine_local_acquisition_score, acquisition_score = self.risklearner_trainer.acquisition_function(candidate_identifier, self.gamma_0, self.gamma_1, self.gamma_2, real_batch_size=real_batch_size)
            return best_batch_id, diversified_score, combine_local_diverse_score, combine_local_acquisition_score, acquisition_score
        else:
            acquisition_score, acquisition_mean, acquisition_std = self.risklearner_trainer.acquisition_function(candidate_identifier, self.gamma_0, self.gamma_1, self.gamma_2, pure_acquisition=True, real_batch_size=real_batch_size)
            return acquisition_score, acquisition_mean, acquisition_std
    ...
    def sample_tasks(self, i_iter, num_tasks, init_dist=None, test=False):
        if self.current_epoch < self.warmup:
            ...
            self.current_epoch += 1
            array_tasks = self.get_array_tasks(candidate_tasks)
            return candidate_tasks, array_tasks, None, None, None
        candidate_tasks = self._env.sample_tasks(int(num_tasks*self.args.sample_ratio))
        best_batch_id, diversified_score, combine_local_diverse_score, combine_local_acquisition_score, acquisition_score = self.get_acquisition_score(i_iter, candidate_tasks, diversified=True, real_batch_size=num_tasks)
        selected_index = best_batch_id
        tasks = [candidate_tasks[i] for i in selected_index]
        array_tasks = self.get_array_tasks(tasks)
        return tasks, array_tasks, diversified_score, combine_local_diverse_score, combine_local_acquisition_score
